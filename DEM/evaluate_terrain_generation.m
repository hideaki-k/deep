function f = evaluate_terrain_generation(k,mode,pix,angle,folder_name,is_noise,is_boulder)
    f = mode;
...
%% 斜面 & フラクタルの付与
   ...
   %% クレータの個数 & 座標を決定
   crater_num = round(1 + (1 + 2)*rand(1)); % クレータ個数(1~6)
   ...
   for crater = 1:1:crater_num
       R(crater) = 7 + (-7 + 15)*rand(1); %クレータ半径(3~10)
   ...
   %% ボルダーの個数 & 座標を決定
    ...
    boulder_num = round(3*rand(1)); % ボルダー個数(0~5)
 ...
 %% true_DEM
    true_DEM = DEM;    
 %% 
    Lidar_noised_DEM = true_DEM + 0.2*randn(64);
